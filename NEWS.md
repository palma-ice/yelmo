...
- MISMIP3D-Stnd validation (dx=20 km, 15 kyr): both the residual and energy solvers agree well with v1.15 — no spurious high-velocity solutions, all shelf velocities well under `ssa_vel_max = 5000`. Residual ≡ v1.15 to roundoff on all summary metrics. Energy differs from residual in the shelf only (max uxy_bar ~2055 vs 1282 m/yr at the calving front, GL ~60 km further upstream), consistent with its variational calving-front BC.
- Fix: `par-gmd/yelmo_MISMIP3D.nml` was missing `nml_ycalv = "ycalv"` in `&yelmo` and `lsf_redist_n_iter` in `&ycalv`, causing the binary to abort during init. Added both.
