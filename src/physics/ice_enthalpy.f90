...
contains 
    ...
    subroutine calc_temp_column_combined(enth,temp,omega,bmb_grnd,Q_ice_b,Q_lith,H_cts,T_pmp,cp,kt,advecxy,uz, &
                                Q_strn,Q_b,Q_geo,T_srf,T_shlf,H_ice,H_lith,H_w,f_grnd,zeta_aa,zeta_ac, &
                                dzeta_a,dzeta_b,omega_max,T0,dt,k0)
        ...
        real(prec), intent(INOUT) :: enth(:)        ! nz_aa [J kg] Total column enthalpy
        real(prec), intent(INOUT) :: temp(:)        ! nz_aa [K] Total column temperature
        real(prec), intent(INOUT) :: omega(:)       ! nz_aa [-] Total column water content fraction
        ...
        integer,    intent(IN)    :: k0             ! Index of ice/bed interface 
        ...
        kappa_aa(k0+1:nz_aa) = kt(k0+1:nz_aa) / (rho_ice*cp(k0+1:nz_aa))
        kappa_aa(1:k0-1)     = kt(1:k0-1)     / (rho_l*cp(1:k0-1))
        ...
        Q_strn_now(1:k0-1)   = Q_strn(1:k0-1)   / (rho_l*cp(1:k0-1))
        ...
        if (f_grnd .lt. 1.0) then
            ...
            val_ice_base      = (f_grnd*T_pmp(k0) + (1.0-f_grnd)*T_shlf)
            ...
            else if ( temp(k0) .lt. T_pmp(k0) .or. H_w_predicted .lt. 0.0_prec ) then
...
!             where (temp .ge. T_pmp) omega = omega_max 
        ...
        if (H_ice .gt. 0.0_prec) then 
            ...
            dz = H_ice * (zeta_aa(k0+1)-zeta_aa(k0))
            Q_ice_b_now = -kt(1) * (temp(k0+1) - temp(k0)) / dz 
        ...
        ! 1st order, upwind gradient dTdz 
        ! Works, but can cause oscillations in H_w 
        dz = H_lith * (zeta_aa(k0)-zeta_aa(k0-1))
        Q_lith_now = -kt(1) * (temp(k0) - temp(k0-1)) / dz 
    ...
    subroutine calc_temp_column_internal_combined(temp,kappa,uz,advecxy,Q_strn,val_base,val_ice_base,val_srf,thickness, &
        ...
        do k = 2, k0-1
            ...
            dz      =  thickness_lith*(zeta_aa(k+1)-zeta_aa(k-1))
        ...
        ! == Lithosphere / ice interface layer k0 ==
...
!             Q_ice_b = kt(1) * (T_ice(2) - T_ice(1) - (0.5_prec*dz*dz*d2Tdz2)) / dz 
        ...
        ! Calculate basal mass balance (valid for grounded ice only)
        call calc_bmb_grounded(bmb_grnd,T_ice(1)-T_pmp(1),Q_ice_b_now,Q_b,Q_lith_now,f_grnd,rho_ice)
    ...
    subroutine calc_enth_column(enth,T_ice,omega,bmb_grnd,Q_ice_b,H_cts,T_pmp,cp,kt,advecxy,uz, &
        ...
        ! Get bedrock surface heat flux in proper units 
        Q_lith_now = Q_lith*1e-3*sec_year   ! [mW m-2] => [J m-2 a-1]

        ! Get basal frictional heating in proper units
        Q_b_now = Q_b*1e-3*sec_year   ! [mW m-2] => [J m-2 a-1]
