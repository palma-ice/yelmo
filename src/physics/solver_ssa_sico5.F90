module solver_ssa_sico5
    ...
    private 
    public :: calc_vxy_ssa_matrix_aa_ac
...
contains 
    ...
    subroutine calc_vxy_ssa_matrix_aa_ac(vx_m,vy_m,L2_norm,beta,visc_int, &
                    ssa_mask,H_ice,f_ice,taud_acx,taud_acy,H_grnd,z_sl,z_bed, &
        ...
        real(prec), intent(IN)    :: beta(:,:)            ! [Pa a m^-1] Basal friction (aa-nodes)
        real(prec), intent(IN)    :: visc_int(:,:)        ! [Pa a m] Vertically integrated viscosity (aa-nodes)
        integer,    intent(IN)    :: ssa_mask(:,:)        ! [--] Mask to determine ssa solver actions (aa-nodes)
        ...
        real(wp) :: inv_dx, inv_dxdx, inv_2dxdx 
        real(wp) :: inv_dy, inv_dydy, inv_2dydy
        real(wp) :: inv_dxdy, inv_2dxdy, inv_4dxdy 
        real(wp)   :: del_sq, inv_del_sq
        real(prec) :: factor_rhs_2, factor_rhs_3a, factor_rhs_3b
        real(prec) :: rho_sw_ice, H_ice_now, taud_now, H_ocn_now
        integer    :: IMAX, JMAX 
        ...
        real(prec), allocatable :: visc_int_acx(:,:) 
        real(prec), allocatable :: visc_int_acy(:,:) 
...
#include "lisf.h"
        ...
        inv_2dxdx = 1.0_wp / (2.0_wp*dx*dx)
        inv_dy    = 1.0_wp / dy 
        inv_dydy  = 1.0_wp / (dy*dy)
        inv_2dydy = 1.0_wp / (2.0_wp*dy*dy)
        ...
        if ( count(H_grnd .gt. 100.0 .and. H_ice .gt. 0.0) .gt. 0 .and. &
                count(beta .gt. 0.0 .and. H_grnd .gt. 100.0 .and. H_ice .gt. 0.0) .eq. 0 ) then  
            ...
            write(*,*) "range(beta):   ", minval(beta), maxval(beta)
        ...
        call map_a_to_cx_2D(visc_int_acx,vis_int_g)
        call map_a_to_cy_2D(visc_int_acy,vis_int_g)
        ...
        !                     end if
                ...
                nc = 2*ij2n(i+1,j)-1        ! column counter for vx_m(i+1,j)
                k = k+1
                lgs_a_value(k) =  2.0_wp*inv_dxdx*visc_int_acx(i,j)
                lgs_a_index(k) = nc
                ...
                k = k+1
                lgs_a_value(k) = -2.0_wp*inv_dxdx*visc_int_acx(i,j) &
                                 -1.0_wp*inv_2dydy*visc_int_acy(i,j) &
                                 -1.0_wp*inv_2dydy*visc_int_acy(i,j-1) &
                                 -0.5_wp*beta(i,j)
                lgs_a_index(k) = nc

                nc = 2*ij2n(i,j)-1          ! column counter for vx_m(i,j)
                k = k+1
                lgs_a_value(k) = -2.0_wp*inv_dxdx*visc_int_acx(i-1,j) &
                                 -1.0_wp*inv_2dydy*visc_int_acy(i,j) &
                                 -1.0_wp*inv_2dydy*visc_int_acy(i,j-1) &
                                 -0.5_wp*beta(i,j)
                lgs_a_index(k) = nc

                nc = 2*ij2n(i-2,j)-1        ! column counter for vx_m(i-2,j)
                k = k+1
                lgs_a_value(k) =  2.0_wp*inv_dxdx*visc_int_acx(i-1,j)
                lgs_a_index(k) = nc
                ...
                nc = 2*ij2n(i,j+1)-1        ! column counter for vx_m(i,j+1)
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_2dydy*visc_int_acy(i-1,j)
                lgs_a_index(k) = nc

                nc = 2*ij2n(i-1,j+1)-1      ! column counter for vx_m(i-1,j+1)
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_2dydy*visc_int_acy(i,j)
                lgs_a_index(k) = nc

                nc = 2*ij2n(i,j-1)-1        ! column counter for vx_m(i,j-1)
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_2dydy*visc_int_acy(i,j-1)
                lgs_a_index(k) = nc
                ...
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_dxdy*visc_int_acx(i,j) &
                                 -1.0_wp*inv_dxdy*visc_int_acx(i-1,j)
                lgs_a_index(k) = nc
                ...
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_dxdy*visc_int_acx(i,j) &
                                 +1.0_wp*inv_2dxdy*visc_int_acy(i,j)
                lgs_a_index(k) = nc
                ...
                nc = 2*ij2n(i-1,j)          ! column counter for vy_m(i-1,j)
                k = k+1
                lgs_a_value(k) = -1.0_wp*inv_dxdy*visc_int_acx(i-1,j) &
                                 -1.0_wp*inv_2dxdy*visc_int_acy(i,j)
                lgs_a_index(k) = nc
                ...
                lgs_b_value(nr) = 0.5_wp*(taud_acx(i,j)+taud_acx(i-1,j))
                lgs_x_value(nr) = vx_m(i,j)
        ...
        !                     end if
                ...
                nc = 2*ij2n(i,j+1)          ! column counter for vy_m(i,j+1)
                k = k+1
                lgs_a_value(k) =  2.0_wp*inv_dydy*visc_int_acy(i,j)
                lgs_a_index(k) = nc
                ...
                k = k+1
                lgs_a_value(k) = -2.0_wp*inv_dydy*visc_int_acy(i,j) &
                                 -1.0_wp*inv_2dxdx*visc_int_acx(i,j) &
                                 -1.0_wp*inv_2dxdx*visc_int_acx(i-1,j) &
                                 -0.5_wp*beta(i,j)
                lgs_a_index(k) = nc

                nc = 2*ij2n(i,j)            ! column counter for vy_m(i,j)
                k = k+1
                lgs_a_value(k) = -2.0_wp*inv_dydy*visc_int_acy(i,j-1) &
                                 -1.0_wp*inv_2dxdx*visc_int_acx(i,j) &
                                 -1.0_wp*inv_2dxdx*visc_int_acx(i-1,j) &
                                 -0.5_wp*beta(i,j)
                lgs_a_index(k) = nc

                nc = 2*ij2n(i,j-2)          ! column counter for vy_m(i,j-2)
                k = k+1
                lgs_a_value(k) =  2.0_wp*inv_dydy*visc_int_acy(i,j-1)
                lgs_a_index(k) = nc
                ...
                nc = 2*ij2n(i+1,j)          ! column counter for vy_m(i+1,j)
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_2dxdx*visc_int_acx(i,j-1)
                lgs_a_index(k) = nc

                nc = 2*ij2n(i+1,j-1)        ! column counter for vy_m(i+1,j-1)
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_2dxdx*visc_int_acx(i,j)
                lgs_a_index(k) = nc

                nc = 2*ij2n(i-1,j)          ! column counter for vy_m(i-1,j)
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_2dxdx*visc_int_acx(i-1,j)
                lgs_a_index(k) = nc
                ...
                nc = 2*ij2n(i,j)-1          ! column counter for vx_m(i,j)
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_dxdy*visc_int_acy(i,j) &
                                 -1.0_wp*inv_dxdy*visc_int_acy(i,j-1)
                lgs_a_index(k) = nc
                ...
                k = k+1
                lgs_a_value(k) =  1.0_wp*inv_dxdy*visc_int_acy(i,j) &
                                 +1.0_wp*inv_2dxdy*visc_int_acx(i,j)
                lgs_a_index(k) = nc
                ...
                nc = 2*ij2n(i,j-1)-1        ! column counter for vx_m(i,j-1)
                k = k+1
                lgs_a_value(k) = -1.0_wp*inv_dxdy*visc_int_acy(i,j-1) &
                                 -1.0_wp*inv_2dxdy*visc_int_acx(i,j)
                lgs_a_index(k) = nc

                nc = 2*ij2n(i-1,j-1)-1      ! column counter for vy_m(i-1,j-1)
                k = k+1
                lgs_a_value(k) = -1.0_wp*inv_dxdy*visc_int_acy(i,j-1) &
                                 -1.0_wp*inv_2dxdy*visc_int_acx(i-1,j)
                lgs_a_index(k) = nc

                lgs_b_value(nr) = 0.5_wp*(taud_acy(i,j)+taud_acy(i,j-1))
                lgs_x_value(nr) = vy_m(i,j)
    ...
    end subroutine calc_vxy_ssa_matrix_aa_ac
