...
contains
    ...
    subroutine calc_bmb_grounded(bmb_grnd,T_prime_b,Q_ice_b_now,Q_b,Q_lith_now,f_grnd,rho_ice)
    ...
    elemental subroutine calc_bmb_grounded_enth(bmb_grnd,T_prime_b,omega,Q_ice_b_now,Q_b,Q_lith_now,f_grnd,rho_ice)
        ! Calculate everywhere there is at least some grounded ice 
        ! (centered aa node calculation)
...
!              + 2.0*strss%tyz*strn%dyz )
        ...
        real(prec), intent(INOUT) :: Q_strn(:,:,:)      ! nx,ny,nz_aa [J a-1 m-3] Heat production
    ...
    subroutine calc_basal_heating_fromaa(Q_b,ux_b,uy_b,taub_acx,taub_acy,beta1,beta2)
        ...
        ! Convert units [J a-1 m-2] => [mW m-2]
        Q_b = Q_b * 1e3 / sec_year 
