        ...
        if ( dt .gt. 0.0 ) then     
            ...
            select case(trim(thrm%par%method))
                ...
                case("temp-combined")
                    ...
                    call calc_ytherm_enthalpy_combined_3D(thrm%now%enth,thrm%now%T_ice,thrm%now%omega,thrm%now%enth_rock,thrm%now%T_rock,thrm%now%bmb_grnd, &
                                thrm%now%Q_ice_b,thrm%now%Q_rock, &
                                thrm%now%H_cts,thrm%now%T_pmp,thrm%now%cp,thrm%now%kt,thrm%par%cp_rock,thrm%par%kt_rock,thrm%now%advecxy,dyn%now%ux,dyn%now%uy,dyn%now%uz,thrm%now%Q_strn, &
                                thrm%now%Q_b,bnd%Q_geo,bnd%T_srf,tpo%now%H_ice,thrm%par%H_rock,tpo%now%z_srf,thrm%now%H_w,thrm%now%dHwdt,tpo%now%H_grnd, &
                                tpo%now%f_grnd,tpo%now%dHicedt,tpo%now%dzsrfdt,thrm%par%z%zeta_aa,thrm%par%z%zeta_ac,thrm%par%z%dzeta_a,thrm%par%z%dzeta_b, &
                                thrm%par%zr%zeta_aa,thrm%par%zr%zeta_ac,thrm%par%zr%dzeta_a,thrm%par%zr%dzeta_b,thrm%par%enth_cr, &
...
!         end if 
    ...
    subroutine calc_ytherm_enthalpy_combined_3D(enth,T_ice,omega,enth_rock,T_rock,bmb_grnd,Q_ice_b,Q_rock,H_cts,T_pmp,cp,kt, &
                        cp_rock,kt_rock,advecxy,ux,uy,uz,Q_strn,Q_b,Q_geo, &
                        T_srf,H_ice,H_rock,z_srf,H_w,dHwdt,H_grnd,f_grnd,dHdt,dzsdt,zeta_aa,zeta_ac,dzeta_a,dzeta_b, &
                        r_zeta_aa,r_zeta_ac,r_dzeta_a,r_dzeta_b,cr,omega_max,dt,dx,solver,solver_advec)
        ...
        real(prec), intent(INOUT) :: enth_rock(:,:,:)    ! [J m-3] Ice enthalpy
        real(prec), intent(INOUT) :: T_rock(:,:,:)   ! [K] Ice column temperature
        ...
        real(prec), intent(OUT)   :: Q_rock(:,:)   ! [J a-1 m-2] Bedrock surface heat flux
        ...
        real(prec), intent(IN)    :: H_rock         ! [m] Ice thickness 
        ...
        real(prec), intent(IN)    :: r_zeta_aa(:)    ! [--] Vertical sigma coordinates (zeta==height), aa-nodes
        real(prec), intent(IN)    :: r_zeta_ac(:)    ! [--] Vertical sigma coordinates (zeta==height), ac-nodes
        ...
        do i = 2, nx-1 
            ...
            if (H_ice_now .le. H_ice_thin) then 
                ...
                call calc_temp_bedrock_column(T_rock(i,j,:),kt_rock,rho_rock,H_rock, &
                                                T_ice(i,j,1),Q_geo(i,j),r_zeta_aa)

                ! Get enthalpy too 
                call convert_to_enthalpy(enth_rock(i,j,:),T_rock(i,j,:),0.0_wp,0.0_wp,cp_rock,0.0_wp)

                ! Calculate heat flux through bed surface from bedrock [mW m-2]
                ...
                call calc_temp_column_combined(enth(i,j,:),T_ice(i,j,:),omega(i,j,:),enth_rock(i,j,:),T_rock(i,j,:), &
                            bmb_grnd(i,j),Q_ice_b(i,j),Q_rock(i,j),H_cts(i,j),T_pmp(i,j,:),cp(i,j,:), &
                            kt(i,j,:),cp_rock,kt_rock,advecxy(i,j,:),uz(i,j,:),Q_strn(i,j,:),Q_b(i,j), &
                            Q_geo(i,j),T_srf(i,j),T_shlf,H_ice_now,H_rock,H_w(i,j),f_grnd(i,j), &
                            zeta_aa,zeta_ac,dzeta_a,dzeta_b,r_zeta_aa,r_zeta_ac,r_dzeta_a, &
                            r_dzeta_b,omega_max,T0,dt)                
        ...
        call fill_borders_3D(enth_rock,nfill=1)
        call fill_borders_3D(T_rock,nfill=1)
